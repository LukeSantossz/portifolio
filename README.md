...
| Client-side IP geolocation + build-time world map for the static Signal section | SSR/edge geolocation; Browser Geolocation API; runtime map library (d3-geo/leaflet) | Keeps the site fully static and runtime lean (no map library), no permission prompt, graceful fallback (author point renders on geo failure). Trade-off: visitor IP reaches a third-party geo API (coarse, not stored). See [ADR-0007](docs/adr/0007-client-side-geolocation-and-build-time-world-map.md). |
