...
| Mobile-only swipe carousel for Case studies (native scroll-snap); the vertical stack on desktop | Pinned scroll-scrub showcase (superseded); a JS transform carousel; the carousel at every width | Below `md`, swipe sideways between case files (where the pinned scrub was invisible), without hijacking scroll; at `md`+ the default readable stack stays. No-JS keeps a native scrollable list. Supersedes ADR-0005. See [ADR-0009](docs/adr/0009-projects-swipe-carousel.md). |
