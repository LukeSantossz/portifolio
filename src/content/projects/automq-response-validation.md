...
evaluation: "No dataset and no metric: the evidence is the test suite, and the merge is a maintainer review rather than a measurement. The tests reproduce the crash on a count mismatch before the guard and assert the server error that replaces it after, which is the whole of the claim. How often the mismatch occurs in practice is not measured and is not claimed."
