...
kind: "personal"
metric: "0.81 AUROC"
metricLabel: "confidence score separating right answers from wrong"
...
constraints: "No budget for paid inference, so everything runs on self-hosted open models. It has to work without reliable internet, because that is where it is used. And the trust signal has to mean something to a technician, not to a developer."
approach: "I built the confidence score on semantic entropy, following Farquhar et al. (Nature, 2024). It samples the same question ten times, clusters the answers by bidirectional entailment using a DeBERTa NLI model, and takes the entropy over those clusters, normalised by log(k) so it lands in 0 to 1. The retrieval side is unremarkable on purpose: Qdrant, FastAPI, Ollama serving Llama 3.1 8B."
